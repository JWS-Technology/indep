...
export default function MyFiles() {
    ...
    // Fetch files after user loads
    useEffect(() => {
        if (user?.coordinator?.assignedEvents) {
            fetchFiles(user.coordinator.assignedEvents);
        }
    }, [user]);

    const fetchFiles = async (assignedEvents: string[]) => {
        try {
            setLoading(true);

            const res = await fetch("/api/user-files", {
                method: "POST",
                body: JSON.stringify({ events: assignedEvents }),
            });

            const json = await res.json();
            setData(json.events || []);
            setFiltered(json.events || []);
        } catch (e) {
            console.error(e);
        } finally {
            setLoading(false);
    ...
    useEffect(() => {
        ...
        const filteredData = data
            .map(ev => ({
                ...ev,
                files: ev.files.filter(f => f.name.toLowerCase().includes(term))
            }))
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-8 px-6">
            <div className="max-w-5xl mx-auto">
                ...
                {/* Page Header */}
                <div className="mb-10">
                    <h1 className="text-4xl font-bold text-gray-900">My Uploaded Files</h1>
                    <p className="text-gray-600 mt-2 text-lg">Files uploaded for your assigned events</p>
                </div>
