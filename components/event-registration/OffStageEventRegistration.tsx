...
export default function OffStageEventRegistration({ eventId: eventIdProp = null }: Props) {
  // State for user data
  ...
  // If eventId prop isn't provided, try to read it from params
  useEffect(() => {
    if (eventIdProp) {
    ...
    try {
      ...
      console.error("Error resolving eventId from params", err);
      setEventId(null);
    }
  }, [params, eventIdProp]);
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        setUserLoading(true);
        const res = await fetch("/api/me", { method: "GET", cache: "no-cache" });
        if (!res.ok) {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      if (!eventId) return;
      try {
        const res = await fetch(`/api/events/${eventId}`, { method: "GET", cache: "no-cache" });
        if (!res.ok) {
          const errorData = await res.json().catch(() => ({}));
  ...
  useEffect(() => {
    const fetchregisteredData = async () => {
      if (!teamId || !eventName || !offStageOrOnStage) return;

      try {
        setLoading(true);
        setprevioslyCreated(false);

        const payload = { eventName, teamId, offStageOrOnStage };
    ...
    const timer = setTimeout(() => fetchregisteredData(), 100);
  ...
  useEffect(() => {
    const getLotNumber = async () => {
      ...
      try {
        const res = await axios.post("/api/get-lot", { eventName, teamId }, { headers: { "Content-Type": "application/json" } });
        setlotNo(res.data.lot?.lot_number || "");
      } catch (err) {
        // optional: ignore lot fetch errors
        console.error("Error fetching lot number", err);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(method === "POST" ? "Registration successful!" : "Registration updated successfully!");
      ...
      // small redirect after success
      setTimeout(() => (window.location.href = "/team/dashboard"), 2000);
    } catch (err: any) {
      ...
      <div className="p-4 md:p-8 flex items-center justify-center bg-gray-100 min-h-screen">
        <div className="w-full max-w-2xl">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="flex gap-4">
              <button onClick={() => window.location.reload()} className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold hover:from-indigo-700 hover:to-purple-700 transition">
                Refresh Page
              </button>
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 mb-8 shadow-xl">
          ...
          <div className="flex flex-wrap gap-4 items-center">
            <div className="bg-white/20 px-4 py-2 rounded-full">
              <span className="text-white font-semibold">{eventName || "Loading event..."}</span>
            </div>
            ...
            {previoslyCreated && (
              <div className="bg-yellow-500/20 px-4 py-2 rounded-full">
                <span className="text-yellow-100 font-semibold">Previously Registered</span>
              </div>
            )}
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-gray-700">Event Name <span className="text-red-500">*</span></label>
                <div className="relative">
                  ...
                  <input type="text" name="eventName" value={eventName || "Loading..."} required readOnly className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl bg-gray-50 text-gray-700 cursor-not-allowed" placeholder="Event name" />
                </div>
              </div>

              {/* Team Name */}
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-gray-700">Team Name <span className="text-red-500">*</span></label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <input type="text" name="teamName" value={teamName || "Loading..."} required readOnly className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl bg-gray-50 text-gray-700 cursor-not-allowed" placeholder="Team name" />
                </div>
              </div>
              ...
              {offStageOrOnStage === "OFF_STAGE" && (
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-gray-700">{eventName === "Quiz - Prelims" ? "First Contestant Name" : "Contestant Name"} <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input type="text" name="contestantName" value={contestantName} onChange={(e) => { const cleaned = e.target.value.replace(/[^A-Za-z ]/g, ""); setcontestantName(cleaned.toUpperCase()); }} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl text-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition disabled:bg-gray-100" placeholder="Enter name" required={offStageOrOnStage === "OFF_STAGE"} disabled={!isRegistrationOpen} />
                  </div>
              ...
              {offStageOrOnStage === "OFF_STAGE" && (
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-gray-700">{eventName === "Quiz - Prelims" ? "First Contestant D.No" : "Contestant D.No"} <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input type="text" name="dNo" maxLength={8} value={dNo} onChange={(e) => setdNo(e.target.value.toUpperCase())} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl text-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition disabled:bg-gray-100" placeholder="Enter D.No" required={offStageOrOnStage === "OFF_STAGE"} disabled={!isRegistrationOpen} />
                  </div>
                </div>
              )}
              ...
              {eventName === "Quiz - Prelims" && offStageOrOnStage === "OFF_STAGE" && (
                <>
                  <div className="space-y-2">
                    <label className="block text-sm font-semibold text-gray-700">Second Contestant Name <span className="text-red-500">*</span></label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                      ...
                      <input type="text" name="secondContestant" value={secondContestant} onChange={(e) => { const cleaned = e.target.value.replace(/[^A-Za-z ]/g, ""); setsecondContestant(cleaned.toUpperCase()); }} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl text-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition disabled:bg-gray-100" placeholder="Enter name" required={eventName === "Quiz - Prelims"} disabled={!isRegistrationOpen} />
                    </div>
                    <p className="text-xs text-gray-500">Format: ADAM BEN A</p>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-semibold text-gray-700">Second Contestant D.No <span className="text-red-500">*</span></label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2"></path>
                      ...
                      <input type="text" name="secondDno" maxLength={8} value={secondDno} onChange={(e) => setsecondDno(e.target.value.toUpperCase())} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl text-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition disabled:bg-gray-100" placeholder="Enter D.No" required={eventName === "Quiz - Prelims"} disabled={!isRegistrationOpen} />
                    </div>
                  </div>
                </>
              )}
              ...
              {/* Lot Number */}
              <div className="mb-6">
                <label className="block font-medium text-gray-700 mb-2">Lot Number</label>
                <div className="flex items-center justify-between bg-indigo-50 border border-indigo-200 rounded-xl p-3 shadow-sm">
                  <span className="text-indigo-700 font-semibold text-lg text-center w-full">{loading ? "Loading..." : lotNo}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-gray-200">
              <button type="submit" disabled={!isRegistrationOpen || loading} className={`w-full py-4 px-6 rounded-xl font-bold text-white transition-all duration-300 transform hover:scale-[1.02] focus:outline-none focus:ring-4 focus:ring-opacity-50 ${!isRegistrationOpen ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 focus:ring-indigo-500"} disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none`}>
